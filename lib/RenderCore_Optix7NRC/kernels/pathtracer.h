...
#endif
void shadeKernelNRC( float4* accumulator, const uint stride,
	...
	const uint pathCount )
{
	...
	if (PRIMIDX == NOHIT)
	{
		if (pixelIdx % 98304 == 0) NRC_DUMP("(JobIndex=%d, pathLength=%d) didn't hit any geometry", jobIndex, pathLength);
		float3 tD = -worldToSky.TransformVector( D );
		float3 skyPixel = FLAGS & S_BOUNCED ? SampleSmallSkydome( tD ) : SampleSkydome( tD );
		float3 contribution = throughput * skyPixel * (1.0f / bsdfPdf);
		CLAMPINTENSITY; // limit magnitude of thoughput vector to combat fireflies
	...
	if ((FLAGS & S_SPECULAR) == 0 && connections != 0) // skip for specular vertices
	{
		if (pixelIdx % 98304 == 0) NRC_DUMP("(JobIndex=%d, pathLength=%d) emit shadow ray", jobIndex, pathLength);
		float pickProb, lightPdf = 0;
		float3 lightColor, L = RandomPointOnLight( r4.x, r4.y, I, fN * faceDir, pickProb, lightPdf, lightColor ) - I;
		// float3 lightColor, L = RandomPointOnLightLTree( r4.x, r4.y, seed, I, fN * faceDir, pickProb, lightPdf, lightColor, false ) - I;
		const float dist = length( L );
		...
		if (pixelIdx % 98304 == 0) NRC_DUMP("(JobIndex=%d, pathLength=%d) NdotL=%f, lightPdf=%f, shadow ray", jobIndex, pathLength, NdotL, lightPdf);
		if (NdotL > 0 && lightPdf > 0)
		{
			float bsdfPdf;
		#ifdef BSDF_HAS_PURE_SPECULARS // see note in lambert.h
...
__global__  __launch_bounds__( 256 /* max block size */, 2 /* min blocks per sm, PASCAL, VOLTA */ )
#endif
void shadeTrainKernel( float4* trainBuf, const uint stride, const uint connectionStride,
	...
	const uint pathCount, float4* debugView, const uint trainingMode, const uint visualizeMode )
{
	// respect boundaries
	int jobIndex = threadIdx.x + blockIdx.x * blockDim.x;
	if (jobIndex >= pathCount) return;
	...
	if ((FLAGS & S_SPECULAR) == 0 && connections != 0) // skip for specular vertices
	{
		...
		NRC_DUMP("(JobIndex=%d, pathLength=%d) NdotL=%f, lightPdf=%f, shadow ray", jobIndex, pathLength, NdotL, lightPdf);
		if (NdotL > 0 && lightPdf > 0)
		{
			float bsdfPdf;
		#ifdef BSDF_HAS_PURE_SPECULARS // see note in lambert.h
	...
	float4* debugView, const uint trainingMode, const uint visualizeMode)
{
	...
	shadeTrainKernel << <gridDim.x, 128 >> > (
		trainBuf, stride, connectionStride, trainPathStates, hits, connections, R0, shift, blueNoise,
		pass, pathLength, scrwidth, scrheight, spreadAngle, pathCount,
		debugView, trainingMode, visualizeMode
	);
}

__host__ void shadeNRC( const int pathCount, float4* accumulator, const uint stride,
	float4* pathStates, float4* hits, float4* connections,
	const uint R0, const uint shift, const uint* blueNoise, const int pass,
...
{
	const dim3 gridDim( NEXTMULTIPLEOF( pathCount, 128 ) / 128, 1 );
	shadeKernelNRC << <gridDim.x, 128 >> > (accumulator, stride, pathStates, hits, connections, R0, shift, blueNoise,
